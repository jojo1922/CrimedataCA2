...
files <- list.files(path ="D:/datascience project/CrimedataCA2",pattern = "*.csv",full.names = T)
# listing the csv files full

All_crime_data <- sapply(files,read_csv,simplify = FALSE) %>% bind_rows(.id="id") # bind all the files to one file called All_Crime_data
nrow(All_crime_data) # number of rows
count(All_crime_data) # count of data set
All_crime_data_new <- subset(All_crime_data , select = -c(1,2,4,5,9,10,12,13)) # remove the following attribute
All_crime_data_new # save to All_crime_data_new
str(All_crime_data_new) # structure of modified file
crime_type <- factor(All_crime_data_new$`Crime type`, ordered = TRUE) # factorise crime type attribute.
crime_type
...
All_crime_data_new$Location <- gsub(paste0(remove_words, collapse = "|"),"",All_crime_data_new$Location)
All_crime_data_new$Location <- toupper(All_crime_data_new$Location)
All_crime_data_new$Location   
na.omit(All_crime_data_new)
view()  
#random_crime_sample <- head(All_crime_data_new,1000)
#random_crime_sample
...
View(randomcrime)
is.na(randomcrime)
clean_1 <- na.omit(randomcrime)
clean_1
#clean_new<- na.omit(new_random, cols = c("Location"))
#clean_new
#cleaned_na <- na.omit(randomcrime, cols = c("Location"))# cleaned the NA values in Location
#cleaned_na
cleaned_na<- subset(clean_1, select = -c(1))# delete the X1 column
cleaned_na
Random_Crime_sample <- head(cleaned_na,1000) # saved the NA values to Random_Crime_sample.
View( Random_Crime_sample)   
# writing function to the post code
find_a_postcode <- function(Location) {
 
 data_filter <- filter(NIPostcodes,NIPostcodes$`Primary Thorfare` == Location)
...
#Random_Crime_sample[Random_Crime_sample$Location==""] <- NA
#View(Random_Crime_sample)
Random_Crime_sample <- na.omit(Random_Crime_sample)# omit na values 
View(Random_Crime_sample)  
chart_data <- subset(Random_Crime_sample, select = -c(1))# eliminate the unwanted column
...
chart_data <- table(chart_data$Longitude,chart_data$Latitude,chart_data$Location,chart_data$Crime.type,chart_data$Postcode)
chart_data
View(chart_data)
#barplot( chart_data, legend.text = T, beside = T,main = 'crime data')

#chart_data # sort the postcode by BT1
